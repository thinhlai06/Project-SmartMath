...
*   **`ai-workflow`**: AI sẽ luôn nhớ dùng `gemma3:12b` (Ollama Cloud) để sinh câu hỏi, `qwen2.5:3b` (local) để chấm bài text + giải thích, và `gemma4:31b` (Cloud OCR) để nhận diện chữ viết tay. Nó cũng nhớ quy tắc: AI output phải luôn là `Draft`.
