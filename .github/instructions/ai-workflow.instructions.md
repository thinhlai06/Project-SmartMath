...
CPA generation now has a structured bundle endpoint for pedagogy-correct output:

- Endpoint: `POST /api/ai/generate-cpa-bundle`
- Persistence: `POST /api/ai/worksheets/{worksheet_id}/cpa-bundles`
- Retrieval: `GET /api/ai/worksheets/{worksheet_id}/cpa-bundles`

Bundle structure is `math_core + concrete + pictorial + abstract` and must pass validator checks before teacher review.
Backward compatibility is preserved with `POST /api/ai/generate-cpa`.
...
Rules for CPA bundle implementation:

- Route by topic family metadata first, then use family-specific generation strategy.
- Arithmetic `math_core` numbers are deterministic by grade/topic rules (AI does not choose final operands).
- Non-arithmetic families use `content_family + family_payload` as core envelope.
- RAG is style guidance only (language/pedagogical phrasing), never arithmetic source of truth.
- All bundle outputs remain `draft` until teacher review/approval.
- Keep AI logic under `backend/app/services/ai/` and use approved model `qwen3:1.7b`.
